...
c
C Adjustments by David Flay (3/31/12) 
C - Made the main program into a subroutine 'F1F209'.  
C - All added variables are in capital letters.  
C - F1F209 takes seven arguments: Z, A, Es, Q2, W2, F1Tot and F2Tot. 
...
C     Added by David Flay 
      REAL*8 Mp,W,P1,P2  
      REAL*8 A,Z,Q2,W2,Es,TH
      REAL*8 F1IN,F2IN,RIN,F1QE,F2QE 
      REAL*8 F1Tot,F2Tot
      REAL*8 SF,SF2 ! Scale factor  
...
C     Unused variables     
      ...
      CALL F1F2IN09(Z,A,Q2,W2,F1IN,F2IN,RIN)
      CALL F1F2QE09(Z,A,Q2,W2,F1QE,F2QE) 

C     introduce a scale factor to get better agreement with E94010 and E01012 
C     determined from a fit of a ratio R = Data/F1F209 for E94010 and E01012 data 
C      SF    = 1.0 
C      SF    = 0.8593 + 0.0135*Es ! first iteration  
      SF    = 0.906 - 0.00699*Es  ! first iteration + E06014 data <-- BEST ONE  
C      SF    = 1.073 - 0.0321*Es   ! second iteration 
C      SF    = 1.031 - 0.01383*Es  ! second iteration + E06014 data
C      SF    = 1.147 - 0.07675*Es  ! iteration after using A*Es + B*W 
C      SF    = 1.0 
C      SF2   = 0.990 - 0.004356*TH  
C      P1    = 7.37244e-02
C      P2    = 5.53252e-01
C      SF    = SF*(P1*Es + P2*W)  
C      SF    = SF*SF2 
